...
class TestCgtSummary(unittest.TestCase):
    ...
    def test_summary(self) -> None:
        ...
        self.assertEqual(3, number_of_disposal(trades, tax_start_date, tax_end_date))
        # share cost = 15000 fee = 175, total allowable cost = 15175
        self.assertEqual(15175, allowable_cost(trades, tax_start_date, tax_end_date))
        # (15000-10000-50-50) + (5000-10) - 3030
        self.assertEqual(
            6860, total_gain_exclude_loss(trades, tax_start_date, tax_end_date)
        )
        # 1500 - 2000 - 15 - 20
        self.assertEqual(-535, capital_loss(trades, tax_start_date, tax_end_date))
        # 15000 + 5000 + 1500
        self.assertEqual(21500, disposal_proceeds(trades, tax_start_date, tax_end_date))
    ...
    def test_date_range(self) -> None:
        ...
        for _, trade_list in trade_bucket.items():
            CgtCalculator(trade_list).calculate_tax()
        self.assertEqual(2, number_of_disposal(trades, tax_start_date, tax_end_date))
        # share cost = 13000 fee = 140, total allowable cost = 13140
        self.assertEqual(13140, allowable_cost(trades, tax_start_date, tax_end_date))
        # (15000-10000-50-50) + (5000-10) - 3030
        self.assertEqual(
            6860, total_gain_exclude_loss(trades, tax_start_date, tax_end_date)
        )
        # the disposal with capital loss is outside tax date range, so 0
        self.assertEqual(0, capital_loss(trades, tax_start_date, tax_end_date))
        # 15000 + 5000 (1500 is outside date range)
        self.assertEqual(20000, disposal_proceeds(trades, tax_start_date, tax_end_date))
