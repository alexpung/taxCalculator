...
def get_number_of_disposal(trades: List[Trade], tax_year_start, tax_year_end) -> int:
...
def get_text_summary(trades: List[Trade], tax_year_start, tax_year_end) -> str:
    """get text summary for the calculated trade list"""
    number_of_disposal = get_number_of_disposal(
        trades,
        tax_year_start,
        tax_year_end,
    )
    disposal_proceeds = get_disposal_proceeds(
        trades,
        tax_year_start,
        tax_year_end,
    )
    allowable_cost = get_allowable_cost(
        trades,
        tax_year_start,
        tax_year_end,
    )
    total_gain = get_total_gain_exclude_loss(
        trades,
        tax_year_start,
        tax_year_end,
    )
    capital_loss = get_capital_loss(
        trades,
        tax_year_start,
        tax_year_end,
    )
